...
# types_out = ['.png', '.jpg', '.jpeg']
# types_bg = ['.png', '.jpg', '.jpeg']
# max_fg = 10
# min_fg = 1
# mask_col = [(255, 0, 0), (0, 255, 0), (0, 0, 255)]
# zpad = 8
...
class ImageComposition():
    """ Composes images together in random ways, applying transformations to the foreground to create a synthetic combined image.
        --------------------------------------------------------------------
        allowed_output_types, allowed_background_types: allowable file formats for images e.g. '.jpg'
        max_foregrounds, min_foregrounds: maximum/minimum number of target objects to place in a synthesized image
        mask_colors: list of distinct RGB values for masks e.g. [(255, 0, 0), (255, 255, 0)] for red and yellow. No. of values must be >= max_foregrounds.
        zero_padding: for output naming. Increasing allows for more output names (default=8)
    """

    def __init__(self, allowed_output_types:list=['.png', '.jpg', '.jpeg'], allowed_background_types:list=['.png', '.jpg', '.jpeg'], max_foregrounds:int=12, min_foregrounds:int=1, mask_colors:list=[(255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0), (255, 0, 255), (0, 255, 255), (125, 125, 0), (125, 0, 125), (0, 125, 125), (125, 125, 125), (255, 125, 0), (0, 125, 255)], zero_padding:int=8):
        self.allowed_output_types = allowed_output_types
        self.allowed_background_types = allowed_background_types
        self.zero_padding = zero_padding # 00000027.png, supports up to 100 million images
        self.max_foregrounds = max_foregrounds
        self.min_foregrounds = min_foregrounds
    ...
    def _generate_images(self):
        ...
        for i in tqdm(range(self.count)):
            ...
            num_foregrounds = max(int(random.triangular(self.min_foregrounds, self.max_foregrounds, mode=0.30*(self.min_foregrounds-self.max_foregrounds))), 1 )  # insure at least 1 fg
            foregrounds = []
    ...
    def _compose_images(self, foregrounds, background_path):
        ...
        for fg in foregrounds:
            ...
            fg_image, fg_area = self._transform_foreground(fg, fg_path)
            ...
            if fg_area > 0.75 * (bg_area/len(foregrounds)):
                scale = (0.75/len(foregrounds))*(bg_area/fg_area)
    ...
    def _transform_foreground(self, fg, fg_path):
        ...
        # Rotate and scale the foreground
        W = fg_image.size[0]
        H = fg_image.size[1]
        angle_degrees = random.randint(-30, 30)
        fg_image = fg_image.rotate(angle_degrees, resample=Image.BICUBIC, expand=1)
        # Adjust rotation scale
        # Adjust rotation scale
        W_prime = fg_image.size[0]
        H_prime = fg_image.size[1]
        # Apply additional random scale
        scale = random.random() * .20 + .75 # Pick between .75 and .95
        new_size = (int(W_prime * W/W_prime * scale), int(H_prime * H/H_prime * scale))
        fg_image = fg_image.resize(new_size, resample=Image.BICUBIC)
